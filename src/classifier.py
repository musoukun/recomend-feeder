...
- 「弊社では」「うちのプロジェクトでは」等、具体的なツール名・手法・数字がなく、匿名の自社事例を語るだけのもの。第三者が再現・活用できる情報がない
- 宣伝・セルフプロモーション（自分のスクールやサービスの宣伝）
- 情報商材的な煽り・収益アピール。「金持ちになる」「稼げる」「月○万」等の金儲け文脈でツールを列挙しているだけのものも含む。技術の使い方やノウハウの具体的解説がない限りskip
- 内輪ネタや文脈がないと意味が通じないリプライ
- 「やってる／やってない」チェックリスト系。ツール名を並べているだけで使い方や知見の共有がないもの
- エンジニアの一般論・持論・心構え系（「ベテランは○○する」「開発で大事なのは○○」等）。新しいツールや技術の具体的な情報がないもの
- イベント・勉強会・LTの告知・参加募集
...
何がすごいのか・何が起きたのかを平易な常体で70文字以内に言い切る。
- 事実と数字で語る。「重要」「画期的」等の評価語や比喩で押し切らない
- 初出の専門用語は平易に言い換える
- 「〜とのこと」「〜らしい」は禁止。言い切る
- skipは空文字でよい
