...
STYLE_RULES = """
## 文体
- カジュアルだが密度が高い。「わかってる人が手短に説明してる」温度感
- 常体（だ・である）ベース。ただし「〜になった」「〜できる」くらいの柔らかさは入れる
...
PROMPT_AI_TECH = f"""Xのツイートを元に、AI・テック関連ニュースのレポートを作成する。

## タイトル
- 20〜30字程度。内容が一言で伝わるもの
- 「～する時代に」「～の登場で○○が変わる」のような軽く引きのあるトーン

## 構成
1. タイトル（##）
2. 冒頭2〜3行で「今日は何が起きたか」を密度高く説明
3. トピック別セクション（番号付き）:
   - 何が出た・何が変わったか（事実）
   - なぜそれが注目か（意味）
   - 読者がイメージできる平易な具体例
...
PROMPT_AI_CAREER = f"""Xのツイートを元に、AI×雇用・働き方に関する論点整理レポートを作成する。

## タイトル
- 20〜30字程度。「AIで○○はどう変わるか」「○○と○○、2つの見方」のようなトーン

## 構成
1. タイトル（##）
2. 冒頭2〜3行で「今日はどんな論点が出ているか」を密度高く俯瞰
3. 論点別セクション（番号付き）:
   - 「こういう見方がある」「一方でこういう意見もある」形式で中立に整理
   - 各意見の背景や根拠を簡潔に添える
   - 編集者自身の結論は出さない
   - ソース: [著者名](URL) 形式
4. 賛否両論ある場合は両方の視点を公平に扱う
5. 1つの意見しかないトピックでもセクションにしてよい
6. 全体で1000〜2000文字
{STYLE_RULES}"""

PROMPT_YOUTUBE = f"""YouTube動画の要約を元に、注目動画のまとめレポートを作成する。

## タイトル
- 20〜30字程度。最も注目の動画内容から取る

## 構成
1. タイトル（##）
2. 冒頭1〜2行で今回の注目ポイントを一言で
3. 動画ごとのセクション（番号付き）:
   - 1行目: 結論を1文で（この動画で一番伝えたいこと）
   - 2行目: 補足があれば1文だけ
   - 最終行: [チャンネル名 - タイトル](URL)
   - 1動画につき最大3行。それ以上書かない
4. 全体で500〜1000文字
