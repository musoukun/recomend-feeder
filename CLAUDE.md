...
加えて YouTube チャンネルの動画を Gemini で要約して RSS + Google Spreadsheet に書き込む機能もある。
データは Google Spreadsheet に蓄積（YouTube要約 / Twitterおすすめ の2シート）。
...
- `classifier.py`: ツイートをバッチ(20件)で Gemini に送り、JSON Schema で構造化レスポンスを得て7カテゴリに分類
- `feed_generator.py`: feedgen で全体フィード(`feed.xml`)+ カテゴリ別フィード(`feed-{category}.xml`)を `docs/` に出力

### YouTube 要約パイプライン (`youtube_main.py`)
```
youtube_summarizer.py (RSS取得 + transcript/Gemini) → スプシ + feed-ai-youtuber.xml
```
- `youtube_summarizer.py`: YouTube RSS → youtube-transcript-api で字幕取得 → Gemini要約（字幕なしの場合URL直接渡しでフォールバック）
- `spreadsheet.py`: GAS Web App (POST) 経由で Google Spreadsheet に書き込み（youtube / twitter 2シート対応）
- `gas/Code.gs`: スプレッドシート側のGASコード（シート別の重複排除）
...
`ai-tech`, `ai-career`, `politics`, `romance`, `adult`, `news`, `other` — `classifier.py` の `Category` enum で定義
...
- `GAS_WEBAPP_URL`: スプレッドシート書き込み用GAS Web App URL
- `YT_MAX_PER_CHANNEL`: YouTubeチャンネルあたりの取得件数 (デフォルト5)
